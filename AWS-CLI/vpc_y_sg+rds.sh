...
apt update && DEBIAN_FRONTEND=noninteractive apt install certbot -y

# CONFIGURACION DE LET'S ENCRYPT (Certbot)
if [ -f "$CERT_PATH" ]; then
    certbot renew --non-interactive --quiet
else
    certbot certonly --standalone -d $DUCKDNS_DOMAIN --non-interactive --agree-tos -m admin@$DUCKDNS_DOMAIN
fi

# FUSIONAR ARCHIVOS DE CERTIFICADO
cat /etc/letsencrypt/live/$DUCKDNS_DOMAIN/fullchain.pem \
/etc/letsencrypt/live/$DUCKDNS_DOMAIN/privkey.pem \
| tee /etc/letsencrypt/live/$DUCKDNS_DOMAIN/haproxy.pem > /dev/null

# DAR PERMISOS AL CERTIFICADO
chmod 644 /etc/letsencrypt/live/$DUCKDNS_DOMAIN/haproxy.pem
chmod 755 -R /etc/letsencrypt/live/$DUCKDNS_DOMAIN
...
echo "${PEM_KEY}" > /home/ubuntu/.ssh/${KEY_NAME}.pem
chmod 400 /home/ubuntu/.ssh/${KEY_NAME}.pem

# Copiar A wordpress, para configurarlo
# scp -i "/home/ubuntu/.ssh/${KEY_NAME}.pem" -r /etc/letsencrypt/live/srestrepoj-wordpress.duckdns.org ubuntu@10.225.4.10:/home/ubuntu
EOF
)
